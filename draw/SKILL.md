...
**生圖完成後，你（AI）必須做以下兩件事，否則用戶看不到圖：**

1. **解析 stderr 印出的「[OK] 路徑」**，例如：
   ```
   [OK] /tmp/xx/generated/image_20260426_143012.jpeg
   ```

2. **立即用 Read tool 讀那個路徑的圖檔**（PNG/JPEG/WebP 都支援），這樣：
   - 你（AI）能實際看到生成的圖長怎樣
   - Claude Code UI 會把圖渲染給用戶看
   - 你能描述圖的內容是否符合用戶要求

**範例對應：**
```
跑完 Bash → stderr 印 [OK] /tmp/test/generated/cat_xxx.jpeg
↓ 立刻 Read("/tmp/test/generated/cat_xxx.jpeg")
↓ AI 看到圖，描述內容，問用戶滿意嗎
```

❌ **錯誤示範**：跑完 bash 只回「✅ 圖已生成在 /tmp/...」就結束 → 用戶看不到任何圖
✅ **正確示範**：跑完 bash → Read 圖檔 → 圖渲染出來 → AI 描述並問用戶意見
