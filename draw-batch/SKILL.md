...
## ⚠️ 重要：跑完一定要看結果（給 AI 看的硬規則）

**批次跑完後，你（AI）必須做這幾件事，否則用戶看不到圖：**

1. **解析 stderr**：每張會印 `[idx] vary_label ...`，最後印 `📁 輸出：<資料夾路徑>`
2. **掃資料夾**：用 `Glob("<那個資料夾>/*.jpeg")` 取得所有檔案路徑
3. **每張都用 Read tool 讀**（PNG/JPEG/WebP 都支援）— 至少前 4 張，用戶才看得到圖
4. **讀 `_summary.md`** 給用戶看每張的 prompt 對照
5. **描述差異**：你看完後告訴用戶「第 1 張是 X 變體、第 2 張是 Y...」並問哪張要 polish

**範例：**
```
跑 draw_batch.py "太空貓" --vary pose --n 4
↓ stderr 印：📁 輸出：/tmp/x/generated/batch_xxx/
↓ Glob("/tmp/x/generated/batch_xxx/*.jpeg") → 抓 4 個路徑
↓ 對 4 個路徑分別 Read → 4 張圖渲染給用戶
↓ Read _summary.md → 列每張對應的 pose
↓ AI 描述差異並問：「哪張要做 reference 模式高一致版本？」
```

❌ 錯：跑完只回「✅ 4 張完成在 /tmp/...」結束 → 用戶看不到任何圖
✅ 對：跑完 → Glob → Read 全部 → 描述 → 問下一步
